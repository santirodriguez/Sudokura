...
files=sorted(args.directory.glob('*.bmp'));assert len(files)==70,f"expected 70 BMPs, found {len(files)}"
...
assert all(value==7 for value in counts.values()),counts
print('validated 70 non-blank BMPs with exact dimensions:',', '.join(f'{w}x{h}' for w,h in sorted(EXPECTED)))
