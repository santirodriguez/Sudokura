...
word=(root/"wordmark.c").read_text(); match=re.search(r"wordmark_width=(\d+),sudokura_wordmark_height=(\d+)",word); assert match and (int(match.group(1)),int(match.group(2)))==(384,107)
values=re.findall(r"(?<![A-Za-z_])([0-9]+),",word.split("[]={",1)[1]);assert len(values)==384*int(match.group(2))*4
print(f"validated 8 PNG sizes, 6-size ICO, ICNS, 128x128 icon and 384x{match.group(2)} wordmark RGBA ({len(values)} encoded bytes)")
