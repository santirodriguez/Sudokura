...
bool game_board_valid(const int b[81]){ for(int i=0;i<81;i++) if(b[i]<1||b[i]>9||!allowed(b,i/9,i%9,b[i])) return false; return true; }
...
int game_solution_count(const int p[81],int limit){ int b[81];memcpy(b,p,sizeof b);return limit>0?solve_count(b,limit):0; }
...
int game_clue_count(const Game*g){int n=0;for(int i=0;i<81;i++)n+=g->fixed[i]!=0;return n;}
bool game_place(Game*g,int r,int c,int v,bool strict){if(r<0||r>8||c<0||c>8||v<0||v>9||g->fixed[IDX(r,c)])return false;if(v&&strict&&!allowed(g->puzzle,r,c,v))return false;g->puzzle[IDX(r,c)]=v;g->notes[IDX(r,c)]=0;return true;}
bool game_toggle_note(Game*g,int r,int c,int v){int i=IDX(r,c);if(r<0||r>8||c<0||c>8||v<1||v>9||g->fixed[i]||g->puzzle[i])return false;g->notes[i]^=(uint16_t)(1u<<v);return true;}
bool game_hint(Game*g,int r,int c){int i=IDX(r,c);if(r<0||r>8||c<0||c>8||g->fixed[i]||g->puzzle[i]==g->solution[i])return false;g->puzzle[i]=g->solution[i];g->notes[i]=0;return true;}
bool game_has_conflict(const Game*g,int r,int c){int v=g->puzzle[IDX(r,c)];return v&&!allowed(g->puzzle,r,c,v);}
int game_conflict_count(const Game*g){int n=0;for(int i=0;i<81;i++)n+=game_has_conflict(g,i/9,i%9);return n;}
bool game_is_solved(const Game*g){return game_board_valid(g->puzzle)&&memcmp(g->puzzle,g->solution,sizeof g->puzzle)==0;}
